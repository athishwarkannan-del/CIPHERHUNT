...
import { Shield, Key, Mail, AlertCircle, CheckCircle2, Eye, EyeOff } from 'lucide-react';
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== confirmPassword) {
      return setError('Credential mismatch! Passwords do not match.');
    }
    ...
    if (result.success) {
      setSuccess('Operator credentials authorized! Redirecting to terminal...');
      setTimeout(() => {
        navigate('/');
      }, 2000);
    } else {
      setError(result.error || 'Failed to initialize credentials.');
      setLoading(false);
    }
  };
      ...
      <div className="relative w-full max-w-lg bg-cyber-card border border-cyber-border rounded-2xl shadow-2xl overflow-hidden z-10">
        ...
        <div className="p-10">
          ...
          <div className="flex flex-col items-center mb-10">
            ...
            <p className="text-xs uppercase font-mono tracking-widest text-cyber-muted/80 mt-1.5">Register Security Credentials</p>
          </div>

          {error && (
            <div className="mb-6 p-4.5 bg-red-950/30 border border-red-500/30 text-red-400 rounded-xl flex items-start gap-3 text-sm">
          ...
          {success && (
            <div className="mb-6 p-4.5 bg-emerald-950/30 border border-emerald-500/30 text-cyber-primary rounded-xl flex items-start gap-3 text-sm">
              <CheckCircle2 className="h-5.5 w-5.5 shrink-0 mt-0.5" />
              <span>{success}</span>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Confirm Password */}
            <div className="space-y-2">
              <label className="text-[13px] uppercase font-mono tracking-widest text-cyber-muted/80 font-bold block">Confirm Password</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4.5 flex items-center pointer-events-none text-cyber-muted/80">
                  <Key className="h-5 w-5" />
                </div>
          ...
          <div className="mt-8 text-center text-sm">
            <span className="text-cyber-muted">Already registered? </span>
            <Link to="/login" className="text-cyber-primary hover:underline font-mono font-semibold">
              Access Terminal
            </Link>
          </div>
        </div>
